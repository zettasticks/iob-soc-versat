...
Need to make the changes needed to use fast type rules.
-- Versat needs to output the files directly to the repo or we copy the files that we generated by calling Versat (although we probably wanna keep stuff based on the makefile)
-- Is it worth it to try to upgrade to py2 while we are in the process of doing this?
...
def ThreadMain(workQueue,resultQueue):
   while(True):
      work = workQueue.get()
